...
export function TaskForm({ open, onClose, onSave, initial, defaultDueDate }: Props) {
  ...
  const [priority, setPriority] = useState<typeof PRIORITIES[number]>(
    (initial?.priority as typeof PRIORITIES[number]) ?? 'medium'
  )
  ...
  const [pattern, setPattern] = useState<typeof PATTERNS[number]>(
    (initial?.recur_pattern as typeof PATTERNS[number]) ?? 'daily'
  )
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-background rounded-xl border shadow-xl">
        <div className="flex items-center justify-between border-b px-5 py-4">
          <h2 className="text-sm font-semibold">{initial ? 'Edit task' : 'New task'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          ...
          <div className="space-y-3">
            ...
            {recurring && (
              <PillGroup options={PATTERNS} value={pattern} onChange={setPattern} colorMap={{
                ...
                weekdays: 'bg-primary/10 text-primary',
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              type="submit"
              disabled={!title.trim()}
              className="flex-1 rounded-lg bg-primary text-primary-foreground px-4 py-2 text-sm font-medium hover:bg-primary/90 disabled:opacity-40 transition-colors"
            >
